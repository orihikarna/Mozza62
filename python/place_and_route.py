...
def wire_exp():
    ...
    mod_exp = 'U1'
    pad_addr = '11'
    via_exp_addr1 = kad.add_via_relative(mod_exp, pad_addr, (0, 4.6), via_size_dat)
    ...
    for i in '12':
        mod_exp = f'U{i}'
        mod_cap = f'C{i}'
        via_exp_cap_vcc[mod_cap] = kad.add_via_relative(mod_cap, '1', (0, 2.7), via_size_pwr)
    ...
    via_exp_nrst_conn = kad.add_via_relative('U2', '14', (1.6, 3.5), via_size_dat)
    via_exp_sck_sda = kad.add_via_relative('U1', '8', (-offset, 2.4), via_size_dat)
    via_exp_sda_sck = kad.add_via_relative('U1', '9', (-offset - 0.6, 4.6), via_size_dat)
    kad.wire_mod_pads([
        # NRST
    ...
    kad.wire_mod_pads([
        ...
        ('U2', via_exp_sda_sck, 'U2', wire_via_exp[0], w_exp, (Dird, 0, [(180, 0.5), +135], r_exp)),
        ('U2', via_exp_sck_sda, 'U2', wire_via_exp[4], w_exp, (Dird, 0, [(180, 0.5), -135], r_exp)),
    ])
...
def wire_rj45_jumpers():
    ...
    for idx, pad in enumerate('268'):
        ...
        for layer in Cu_layers:
            ...
            kad.wire_mod_pads([(mod_jmp, '2', mod_jmp, wire_via_rj45[name], w_conn, (Dird, 45, 0), layer)])
    via_rj45['4'] = kad.add_via_relative('JPF4', '2', (offset_x - 0.4, -sep_jmp_y), via_size_pwr)
    kad.wire_mod_pads([
        # '2' -- '8' arc
        ('JPF8', wire_via_rj45['F8'], 'JPB8', wire_via_rj45['B8'], w_conn, (Dird, [(0, sep_via_y * 3), 90], 0), 'B.Cu'),
        # connection via (4)
        ('JPF4', '2', 'JPF4', via_rj45['4'], w_conn, (Dird, [(90, 0.35), 0], 45), 'F.Cu'),
        ('JPB4', '2', 'JPB4', via_rj45['4'], w_conn, (Dird, [(90, 0.35), 0], 45), 'B.Cu'),
        # connect 1 == 9
    ...
    for pidx, pad in enumerate('26'):
        ...
        for lidx, layer in enumerate('FB'):
            ...
            kad.wire_mod_pads([(mod_jmp, wire_via_rj45[name], mod_jmp, via_rj45[pad], w_conn, (Dird, 0, wire_angle), f'{layer}.Cu')])
...
def wire_rj45_vert_lines():
    ...
    for idx, (width, space, net_name, pad) in enumerate(rj45_vert_width_spc_net_pads):
        ...
        if idx < 5:
            ...
            pos = kad.calc_pos_from_pad(mod_rj, '9', (x, -2))
    ...
    # wire GND/VCC vias
    ctr_pwr = kad.calc_relative_vec(mod_rj, (-2, -3.85), kad.get_via_pos_net(wire_via_rj45_row[6])[0])
    r_rj = 2.0
    kad.wire_mod_pads([
        # 6 - GND & 8 - VCC
        ('J1', via_rj45['6'], 'J1', wire_via_rj45_row[5], w_conn, (Dird, [(30, 1.6), 0], [(90, ctr_pwr), 135], r_rj), 'F.Cu'),
        ('J1', via_rj45['8'], 'J1', wire_via_rj45_row[6], w_conn, (Dird, 0, [(90, ctr_pwr), 135], r_rj), 'F.Cu'),
        # Gnd - Gnd
        (mod_rj, wire_via_rj45_row[5], mod_rj, via_gnd, w_conn, (Dird, 90, 0), 'B.Cu'),
...
def remove_temporary_vias():
    ...
    for idx, via in via_led_left.items():
        if idx in ['21']:
            pcb.Delete(via)
    for vias in [wire_via_dbnc_vcc, wire_via_dbnc_gnd, wire_via_dio_col, wire_via_led_pwr_1st, wire_via_led_pwr_2nd]:
